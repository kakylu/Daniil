...
KEY = '1001011001010101001100100111100101100100111111'
...
def slice_mess(string):
            #сразу буковки перевожу в 16 систему
...
def permute(block, box):
            #использую таблицу перестановки
...
def f(bl, key):
            final = []
            for j, i in enumerate(wrap(XOR(permute(bl, EXPANSION), key), 6)):
                  temp_box = [
                        SUB_BOX[j][0:16],
                        SUB_BOX[j][16:32],
                        SUB_BOX[j][32:48],
...
def des(bl, keyarr):
            left, right = bl[0: len(bl) // 2], bl[len(bl) // 2:]
            for j, i in zip(range(1, 17), keyarr):
...
def main(mess,mod):
            if mod == 0:
                  ...
                  for i in slice_mess(mess):
                        binm, bink = to_bin(i), to_bin(KEY)

                        permuted_key, permuted_block = permute(
                              bink, PERMUTED_CHOICE_1), permute(binm, INITIAL_PERMUTATION)

                        key_list = key_gen(
                              permuted_key[: len(permuted_key) // 2], permuted_key[len(permuted_key) // 2:])

                        list.append(''.join([bin(int(i, 2))[2:].zfill(2).upper() for i in des(permuted_block, key_list)]))
                        list1.append(''.join([hex(int(i, 2))[2:].zfill(2).upper() for i in des(permuted_block, key_list)]))
                  ...
                  for i in wrap(mess, 16):
                        ...
                        li.append(''.join([hex(int(i, 2))[2:].zfill(2).upper()
                                                for i in des(permuted_block, reversed(key_list))]))

                        li1.append(''.join([bin(int(i, 2))[2:].zfill(2).upper()
                                                for i in des(permuted_block, reversed(key_list))]))

                  print(''.join(concatenate(
                        [[chr(int(j, 16)) for j in wrap(i, 2) if int(j, 16) != 0] for i in li])))
...
if __name__ == '__main__':
    while True:
        main(input('Введите текст: '), int(input('Нажмите чтобы зашифровать 0 \n'
                                                 'Нажмите чтобы расшифровать 1 \n')))
